...
function searchData(searchModel, searchOption) {
    ...
    return fetch(`${API_URL}/Product/searchData`, configOption(query)).then(response);
}

// Create new data
function createModel(model) {
    return fetch(`${API_URL}/Product/createModel`, configOption(model)).then(response);
}

// Update data
function updateModel(model) {
    return fetch(`${API_URL}/Product/updateModel`, configOption(model)).then(response);
}

// Delete data
function deleteModel(listID) {
    if (Array.isArray(listID) && listID.length > 0) {
        return fetch(`${API_URL}/Product/deleteModel`, configOption({ listID })).then(response);
    } else {
        return false;
    }
}
...
function getByID(id) {
    ...
    return fetch(`${API_URL}/Product/getByID/${id}`, option).then(response);
}
...
function getDataFilter(searchModel) {
    ...
    return fetch(`${API_URL}/Product/getDataFilter`, configOption(query)).then(response);
}
