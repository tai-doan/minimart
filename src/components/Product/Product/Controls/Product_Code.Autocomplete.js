...
const Product_CodeAutocomplete = ({ value, className, name, label, required, message, allowClear, onSelect }) => {
    ...
    function convertDataToOptions(data) {
        return data.reduce((array, item) => {
            return [...array,
            ...[renderItem(item.Product_Code, item.Product_Name)]
            ];
        }, []);
    }
    ...
    const handleSearch = values => {
        ...
        Promise.all([Product_Service.getDataFilter(searchModel)]).then(result => {
...
export { Product_CodeAutocomplete };
