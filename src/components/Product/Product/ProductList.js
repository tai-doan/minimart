...
import { getColumnInSearchTable, defaultColumnInResultTable, defaultFieldInSearchForm, setColumnInSearchTable, getFieldSearch, setFieldSearch, getFieldSort, setFieldSort, searchDefaultModel } from './Models/Product.Models';
import { Product_Search } from './ProductSearch';
import { ProductAddClone } from './ProductAddClone';
...
const ProductList = () => {
    ...
    const onSearchForm = (value, option) => {
        ...
        Promise.all([Product_Service.searchData(value, !!option ? option : searchOption)]).then(searchResult => {
    ...
    const onDeleteConfirm = (title, content, IdList) => {
        confirm({
            ...
            onOk() {
                return Promise.all([Product_Service.deleteModel(IdList)]).then(resultDelete => {
    ...
    return (
        <Layout style={{ padding: 20 }}>
            <Tns_Breadcum module={TNS_MODULE.PRODUCT} screen={language.PRODUCT_SEARCH_PAGE} mainLanguage={mainLanguage} />
            <Content className='site-layout-background'>
                <Product_Search searchField={fieldOnSearchForm}
                    value={searchModel}
