...
import { getColumnInSearchTable, defaultColumnInResultTable, defaultFieldInSearchForm, setColumnInSearchTable, getFieldSearch, setFieldSearch, getFieldSort, setFieldSort, searchDefaultModel } from './Models/Customer.Models';
import { Customer_Search } from './CustomerSearch';
import { CustomerAddClone } from './CustomerAddClone';
...
const CustomerList = () => {
    ...
    const onSearchForm = (value, option) => {
        ...
        Promise.all([Customer_Service.searchData(value, !!option ? option : searchOption)]).then(searchResult => {
    ...
    const onDeleteConfirm = (title, content, IdList) => {
        confirm({
            ...
            onOk() {
                return Promise.all([Customer_Service.deleteModel(IdList)]).then(resultDelete => {
    ...
    return (
        <Layout style={{ padding: 20 }}>
            <Tns_Breadcum module={TNS_MODULE.COMPANY} screen={language.CUSTOMER_SEARCH_PAGE} mainLanguage={mainLanguage} />
            <Content className='site-layout-background'>
                <Customer_Search searchField={fieldOnSearchForm}
                    value={searchModel}
                ...
                <span className='main-screen-button-area'>
                    ...
                    <CustomerAddClone allowAccess={true}
                    ...
                    <CustomerEditView allowAccess={true}
