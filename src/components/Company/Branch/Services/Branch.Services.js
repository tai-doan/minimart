...
function searchData(searchModel, searchOption) {
    ...
    return fetch(`${API_URL}/Branch/searchData`, configOption(query)).then(response);
}

// Create new data
function createModel(model) {
    return fetch(`${API_URL}/Branch/createModel`, configOption(model)).then(response);
}

// Update data
function updateModel(model) {
    return fetch(`${API_URL}/Branch/updateModel`, configOption(model)).then(response);
}

// Delete data
function deleteModel(listID) {
    if (Array.isArray(listID) && listID.length > 0) {
        return fetch(`${API_URL}/Branch/deleteModel`, configOption({ listID })).then(response);
    } else {
        return false;
    }
}
...
function getByID(id) {
    ...
    return fetch(`${API_URL}/Branch/getByID/${id}`, option).then(response);
}
...
function getDataFilter(searchModel) {
    ...
    return fetch(`${API_URL}/Branch/getDataFilter`, configOption(query)).then(response);
}
