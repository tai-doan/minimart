...
import { getColumnInSearchTable, defaultColumnInResultTable, defaultFieldInSearchForm, setColumnInSearchTable, getFieldSearch, setFieldSearch, getFieldSort, setFieldSort, searchDefaultModel } from './Models/User.Models';
import { User_Search } from './UserSearch';
import { UserAddClone } from './UserAddClone';
...
const UserList = () => {
    ...
    const onSearchForm = (value, option) => {
        ...
        Promise.all([User_Service.searchData(value, !!option ? option : searchOption)]).then(searchResult => {
    ...
    const onDeleteConfirm = (title, content, IdList) => {
        confirm({
            ...
            onOk() {
                return Promise.all([User_Service.deleteModel(IdList)]).then(resultDelete => {
    ...
    return (
        <Layout style={{ padding: 20 }}>
            <Tns_Breadcum module={TNS_MODULE.SYSTEM} screen={language.USER_SEARCH_PAGE} mainLanguage={mainLanguage} />
            <Content className='site-layout-background'>
                <User_Search searchField={fieldOnSearchForm}
                    value={searchModel}
