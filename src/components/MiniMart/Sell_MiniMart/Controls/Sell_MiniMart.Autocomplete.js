...
const Sell_MiniMart_Header_CodeAutocomplete = ({ value, className, name, label, required, message, allowClear, onSelect }) => {
    ...
    function convertDataToOptions(data) {
        return data.reduce((array, item) => {
            return [...array,
            ...[renderItem(item.Sell_MiniMart_Header_Code, item.Sell_MiniMart_Header_Name)]
            ];
        }, []);
    }
    ...
    const handleSearch = values => {
        ...
        Promise.all([Sell_MiniMart_Header_Service.getDataFilter(searchModel)]).then(result => {
...
export { Sell_MiniMart_Header_CodeAutocomplete };
